...
public class MovieService {
    ...
    public List<MovieResponse> findMoviesByGenre(String genre) {
        ...
        List<Movie> moviesList = movieRepository.findByGenre(Genre.valueOf(genre));

        if (!CollectionUtils.isEmpty(moviesList)) {
            List<MovieResponse> movieResponseList = moviesList.stream().sorted(Comparator.comparing(Movie::getRating, Comparator.reverseOrder())).map(m -> m.toMovieResponse()).collect(Collectors.toList());
